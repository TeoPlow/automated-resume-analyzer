...
| **Ollama**                | Локальная LLM для парсинга резюме         | Запуск модели локально, нет зависимости от внешних API                   |
...
| `archived` | Архивная. Не отображается в списках по умолчанию. Хранится для истории и аналитики.           | конечный статус       |
...
## Исследования в дипломе

Темы, которые можно подробно раскрыть как исследовательскую часть ВКР.

### 1. Сравнительный анализ LLM-моделей для парсинга резюме

Провести эксперимент: взять 30–50 реальных резюме, разметить эталонные поля
вручную (золотой стандарт). Прогнать через несколько моделей (qwen2.5:7b, qwen3:8b,
gema2:9b, llama3.1:8b) и сравнить:
- **Точность** извлечения по полям (F1-score для навыков, точное совпадение для контактов)
- **Стабильность JSON** — доля валидных ответов с первой попытки
- **Скорость** — среднее время обработки одного резюме
- **Потребление ресурсов** — RAM, CPU, GPU

Результат: таблица сравнения + обоснование выбора модели.

### 2. Эффективность гибридного парсинга (правила + LLM) против чисто-LLM подхода

Сравнить три подхода на одном наборе резюме:
- a) Только regex/правила
- b) Только LLM (всё через модель)
- c) Гибрид (regex для контактов, LLM для сложных полей)

Метрики: точность контактов (exact match), полнота навыков (recall),
общая устойчивость (% успешных обработок без ошибок).

### 3. Калибровка порога семантического сравнения навыков

Порог cosine similarity = 0.6 выбран эмпирически. Исследовать:
- Как меняется precision/recall при порогах 0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9
- Найти оптимальный порог на наборе IT-навыков
- Продемонстрировать примеры ложно-положительных и ложно-отрицательных соответствий
- Построить ROC-кривую

### 4. Сравнение embedding-моделей для matching навыков

Сравнить на наборе пар (навык кандидата, требование вакансии, ручная оценка):
- `paraphrase-multilingual-MiniLM-L12-v2` (текущий выбор)
- `all-MiniLM-L6-v2` (только английский)
- `multilingual-e5-large` (больше и тяжелее, но точнее)
- `cohere-embed-multilingual-v3` (если есть API-ключ)

### 5. Калибровка весов многофакторной модели matching

Веса факторов (0.40 / 0.25 / 0.15 / 0.10 / 0.10) должны быть обоснованы:
- Опрос HR-специалистов (анкетирование): какой фактор важнее?
- Сравнение ранжирования системы с ранжированием HR (на 10–15 примерах)
- Метрика: nDCG (normalized discounted cumulative gain) — насколько
  порядок системы совпадает с порядком эксперта

### 6. Анализ производительности pipeline обработки

Замерить время каждого этапа pipeline на 100 резюме:
- Валидация + загрузка в MinIO
- Извлечение текста (PDF vs DOCX vs TXT)
- Парсинг regex-правилами
- Парсинг LLM (бутылочное горло — самый медленный этап)
- Дедупликация + профилирование
- Matching (узкое место — sentence-transformers на большом числе кандидатов)

Построить график распределения времени по этапам, выявить бутылочные горлышка.

### 7. Сравнение архитектурных подходов: монолит vs микросервисы

Обосновать выбор микросервисной архитектуры:
- Таблица сравнения: сложность разработки, масштабируемость, независимость
  деплоя, отказоустойчивость
- Почему 5 сервисов, а не 3 или 10 — принципы разделения ответственности

### 8. RabbitMQ vs Kafka: обоснование выбора брокера сообщений

- Сравнение по критериям: throughput, гарантии доставки, операционная
  сложность, потребление ресурсов, реплэй событий
- Почему RabbitMQ достаточен при данном объёме событий

### 9. Анализ качества дедупликации кандидатов

- Насколько точно работает дедупликация по email/телефону
- Когда она не работает (нет контактов в резюме, разные контакты)
- Возможные улучшения: fuzzy matching по ФИО, сравнение опыта

### 10. Обзор существующих решений

Сравнить существующие продукты для автоматизации HR:
- Lever, Greenhouse, HuntFlow, Talantix, Работа.ру для работодателей
- Что умеют, какие алгоритмы используют (открытые данные)
- Преимущества собственного решения: данные остаются на своём сервере,
  нет подписки, полный контроль над алгоритмом matching
