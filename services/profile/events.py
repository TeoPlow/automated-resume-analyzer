...
class ProfileEventPublisher:
    def __init__(self) -> None:
        ...
        self._producer_name = os.getenv("PROFILE_EVENT_PRODUCER", "profile-service")
        self._required = os.getenv("PROFILE_EVENTS_REQUIRED", "false").lower() in {"1", "true", "yes", "on"}
        ...
        try:
            ...
            if self._required:
                raise make_http_exception(
                    ...
                    message="Failed to publish profile event",
...
publisher: ProfileEventPublisher | None = None


def get_event_publisher() -> ProfileEventPublisher:
    global publisher
